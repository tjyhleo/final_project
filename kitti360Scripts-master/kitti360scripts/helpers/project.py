...
class Camera:
    ...
    def __call__(self, obj3d, frameId):
        # project 3d points into images and visualise
...
class CameraPerspective(Camera):
    ...
    def load_intrinsics(self, intrinsic_file):
        ...
        #get intrinsic matrix (self.K), rectification matrix (self.R_rect), 
        #image width and height(self.width, self.height) from "perspective.txt"
...
if __name__=="__main__":
    ...
    for frame in camera.frames:
        ...
        for k,v in annotation3D.objects.items():
            if len(v.keys())==1 and (-1 in v.keys()): # show static only
                ...
                if not id2label[obj3d.semanticId].name=='building': # show buildings only
                    continue
                camera(obj3d, frame) #project 3d points into images and visualise
                vertices = np.asarray(obj3d.vertices_proj).T #vertices_proj is (uv), the coordinate in image of the projected 3d point
        ...
        #every ploted image stay for 0.5 seconds then it get cleaned
        plt.pause(0.5)
        plt.clf()
